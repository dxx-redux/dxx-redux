...
3. Contingency (spec §"Why the negated right-vector flips the image"): if the acceptance run on a **software** (non-OGL) build shows corrupted geometry in the mirror, wrap the `vm_vec_negate` call in `#ifdef OGL` so software builds get an unflipped mirror. Do not do this preemptively — only on observed breakage; the default OGL build is winding-safe.
