# Weapon-powerup 3D models — design

Status: approved design · branch `unity` · 2026-07-12

## 1. Context

In Descent 1, weapon powerups on the map (laser, vulcan, quad, missiles, …) are
**pre-rendered turntable sprites**: the artists at Parallax built 3D models, rendered
them spinning about the vertical axis into a `VClip` (animated 2D bitmap sequence), and
shipped only the sprites. The source 3D models were never released and are **not** in the
game data (the PIG's 85 POF models are ships/robots/reactor/mines — no powerups).
...
four-barrel cluster with a pinkish yoke — confirming these are genuine 3D objects.

This feature **restores** the lost 3D models: each weapon powerup becomes a real 3D mesh
that idle-spins in the level, sourced as AI-generated glTF/GLB art through an override
layer, with the existing sprite as automatic fallback.
...
**Goals**
- Weapon powerups render as real 3D meshes that idle-spin, looking as close as feasible to
  the original sprites.
- Art is **AI-generated GLB**, dropped into a per-weapon override slot; adding/replacing a
  model is a file drop with **zero code change**.
- One mechanism covers **all weapon powerups**; laser is the fully-worked first example.
...
- Non-weapon powerups (energy, shield, keys, radar/map, cloak, turbo, invuln, headlight)
  stay sprites.
- No PBR-lit look by default — the game is unlit/vertex-lit; override models match that.
- Not touching the in-flight weapon *projectile* models (missiles fired) or the ship's
  gun — only the **pickups on the map**.

## 3. Scope — the weapon set

Keyed by powerup id (`ObjectRecord.SubtypeId`, constants from `d1/main/powerup.h`):

| Powerup id | Constant | Sprite (frame-0 bitmap) | Override base name |
|---|---|---|---|
| 3  | `POW_LASER`            | `laser`    | `laser` |
| 12 | `POW_QUAD_FIRE`        | `quad`     | `quad` |
| 13 | `POW_VULCAN_WEAPON`    | `vulcan`   | `vulcan` |
| 14 | `POW_SPREADFIRE_WEAPON`| `spread`   | `spread` |
| 15 | `POW_PLASMA_WEAPON`    | `plasma`   | `plasma` |
| 16 | `POW_FUSION_WEAPON`    | `fusion`   | `fusion` |
| 10 | `POW_MISSILE_1`        | `cmissil1` | `cmissile` |
| 11 | `POW_MISSILE_4`        | `cmissil2` | `cmissile4` (may reuse `cmissile`) |
| 18 | `POW_HOMING_AMMO_1`    | `hmissil1` | `hmissile` |
| 19 | `POW_HOMING_AMMO_4`    | `hmissil2` | `hmissile4` (may reuse `hmissile`) |
| 17 | `POW_PROXIMITY_WEAPON` | `pbombs`   | `pbomb` |
| 20 | `POW_SMARTBOMB_WEAPON` | `smissile` | `smart` |
| 21 | `POW_MEGA_WEAPON`      | `mmissile` | `mega` |
| 22 | `POW_VULCAN_AMMO`      | `vammo`    | `vammo` |

The id→name table lives in one place (`OverrideModels`) and is the single source of truth
for what counts as a "weapon powerup." The 4-packs (11, 19) may supply their own GLB or, if
absent, fall through to the single-weapon model, then to their own sprite.
...
LevelViewer dispatch (Presentation, 2 sites) ── NEW guard:
        │      if weapon powerup AND OverrideModels.Has(id):
...
Owns the weapon override system.
...
- **Load:** glTFast `GltfImport` from file bytes → instantiate to a `GameObject`
  hierarchy; combine into shared, cached geometry (one load per weapon, reused across
  instances, mirroring `ModelFactory`'s caching).
- **Auto-fit:** compute renderer bounds → recenter to pivot → uniform-scale so max extent ≈
  `2 × Powerup.Size` (the sprite's on-map diameter; `Powerup.Size` from the pig, e.g. laser
  = 3.0). Optional per-model rotation offset from an adjacent `powerup_<name>.json` manifest.
...
- Both dispatch sites: `if (isWeaponPowerup && overrideModels.TryInstantiate(id, light) is
  {} go) { place + add SpinModel; } else { existing sprite path; }`.
- Dispose with the rest on teardown.

### 6.4 Reference-kit tool (new, editor menu + extends the extractor)
Produces per-weapon AI-generator input into `overrides/refkit/<name>/`:
...
- `view_broadside.png`, `view_quarter.png`, `view_muzzle.png` — the clearest single frames,
  upscaled (nearest-neighbor, ~8×).
- `base.glb` — silhouette-carved base mesh (shape-from-silhouette over the 15 known
  turntable angles + projected pixel skin). Used as the generator's structural seed and to
  synthesize the top view the turntable lacks. **Not shipped in-game.**
...
15-view visual hull captures horizontal cross-sections and height well, with soft top/bottom
and thin features — acceptable for AI seeding.
...
- Any orientation — a `powerup_<name>.json` may specify `rotationEuler` and `scaleMul`
  overrides; default assumes +Y up, +Z forward (barrel), which the reference kit documents.
...
Every failure mode degrades to the existing sprite, logged once per weapon:
...
- **Fallback proof:** remove a weapon's GLB → its sprite returns, no errors.
- **Regression:** non-weapon powerups and all other objects render unchanged; a level with
  no override dir is pixel-identical to today.
...
- **F1 — Pipeline.** `OverrideModels` + `SpinModel` + both dispatch guards + auto-fit +
  material rebind. AC: a throwaway weapon GLB spins in place at correct scale; removing it
  restores the sprite; `PresentationCheck` green.
...
- **F3 — Roll-out.** Reference kits for the remaining weapons; user generates; integrate;
  confirm the full §3 set (decide 4-pack reuse vs. own model).
- **F4 — Modding + polish.** External user mod dir wired; per-weapon spin/scale tuning;
...
| Carved base mesh too rough to seed the generator | it seeds only; the upscaled sprite views are the primary reference; carve is optional per weapon |
| Scope creep into non-weapon powerups / projectiles | §3 table is the explicit allowlist; everything else stays as-is |
```
